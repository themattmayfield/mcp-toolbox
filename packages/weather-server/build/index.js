#!/usr/bin/env node
import { CoordinatesSchema, createErrorResponse, createMcpServer, createSuccessResponse, formatCoordinates, makeApiRequest, StateCodeSchema, validateStateCode, } from "@mcp-toolbox/shared";
const NWS_API_BASE = "https://api.weather.gov";
const USER_AGENT = "weather-app/1.0";
// Pure functions for business logic
const makeNWSRequest = (url) => makeApiRequest(url, {
    headers: {
...
async function main() {
    ...
    server.tool("get_forecast", "Get weather forecast for a location", {
        ...
        try {
            ...
            if (!pointsData) {
                return createErrorResponse(`Failed to retrieve grid point data for coordinates: ${latitude}, ${longitude}. This location may not be supported by the NWS API (only US locations are supported).`);
            }
            const forecastUrl = pointsData.properties?.forecast;
            if (!forecastUrl) {
                return createErrorResponse("Failed to get forecast URL from grid point data");
            }
