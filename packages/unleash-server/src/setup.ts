...
class InteractiveSetup {
	private instances: UnleashInstance[] = [];
	...
	async run(): Promise<void> {
		...
		console.log(
			"You can connect to multiple Unleash instances (production, staging, etc.)\n",
		);
	...
	private async handleExistingConfig(
		config: UnleashConfig,
	): Promise<"continue" | "exit"> {
		console.log(
			`✅ Found existing configuration with ${config.instances.length} instance(s)\n`,
		);
	...
	private async setupInstance(
	...
	): Promise<UnleashInstance> {
		const instanceNumber = this.instances.length + 1;
		console.log(
			`📋 Setting up ${type === "first" ? "your first" : `instance #${instanceNumber}`} Unleash instance\n`,
		);
		...
		if (testResult.success) {
			...
			if (testResult.projects && testResult.projects.length > 0) {
				console.log(
					`📊 Found ${testResult.projects.length} projects: ${testResult.projects.join(", ")}`,
				);
			}
	...
	private async askAddAnother(): Promise<boolean> {
		console.log(
			`✅ Instance '${this.instances[this.instances.length - 1].name}' configured successfully!\n`,
		);
	...
	private async saveConfiguration(): Promise<void> {
		const config: UnleashConfig = { instances: this.instances };
	...
	private async showNextSteps(): Promise<void> {
		...
		console.log(
			`   - create_feature instance="${this.instances[0].name}" name="test-feature"`,
		);
	}

	private async editExistingInstances(
		_instances: UnleashInstance[],
	): Promise<void> {
	...
	private async testAllConnections(
	...
	private showCurrentConfig(config: UnleashConfig): void {
