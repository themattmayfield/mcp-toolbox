...
// Helper functions
function getUnleashInstance(instanceName: string): UnleashInstance {
	const instance = UNLEASH_CONFIG.instances.find(
		(i) => i.name === instanceName,
	);
	if (!instance) {
		throw new Error(
			`Unknown Unleash instance: ${instanceName}. Available instances: ${UNLEASH_CONFIG.instances.map((i) => i.name).join(", ")}`,
		);
	}
	return instance;
}

function getProjectName(instance: UnleashInstance, project?: string): string {
...
async function main() {
	...
	server.tool(
		"list_instances",
		"Show all configured Unleash server instances (e.g., production, staging, development). Use this to see what instances are available.",
		{},
		async () => {
			try {
	...
	server.tool(
		"create_feature",
		"Create a new feature flag and explicitly disable it in all configured environments for the instance. Ensures consistent environment-specific state from creation.",
		CreateFeatureSchema.shape,
		async ({ instance, name, description, type, project }) => {
			try {
				...
				if (environments.length > 0) {
					...
					const disablePromises = environments.map(async (env) => {
						try {
							await makeUnleashRequest(
								unleashInstance,
								`/projects/${projectName}/features/${name}/environments/${env}/off`,
								{
									method: "POST",
								},
							);
							...
							console.error(
								`Failed to disable feature '${name}' in environment '${env}':`,
								error,
							);
					...
					if (failed.length > 0) {
						...
						return createSuccessResponse(
							`✅ Feature flag '${name}' created successfully in ${instance} (${projectName} project)\n⚠️ Warning: Failed to disable in environments: ${failedEnvs}`,
						);
					}

					const envList = environments.join(", ");
					return createSuccessResponse(
						`✅ Feature flag '${name}' created successfully in ${instance} (${projectName} project) and disabled in all environments: ${envList}`,
					);
				} else {
					return createSuccessResponse(
						`✅ Feature flag '${name}' created successfully in ${instance} (${projectName} project)`,
				...
				return createErrorResponse(
					`Failed to create feature flag: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		},
	);
	...
	server.tool(
		"archive_feature",
		"Archive (soft delete) a feature flag when it's no longer needed. Archived flags are hidden from normal views but can be restored if needed.",
		ArchiveFeatureSchema.shape,
		async ({ instance, name, project }) => {
			try {
				const unleashInstance = getUnleashInstance(instance);
				const projectName = getProjectName(unleashInstance, project);
				...
				const result = await makeUnleashRequest(
					unleashInstance,
					`/projects/${projectName}/features/${name}`,
					{
						method: "DELETE",
					},
				);
				...
				return createSuccessResponse(
					`🗄 Feature flag '${name}' archived successfully in ${instance} (${projectName} project)`,
				);
			} catch (error) {
				console.error("Error archiving feature:", error);
				return createErrorResponse(
					`Failed to archive feature flag: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		},
	);
	...
	server.tool(
		"get_feature_status",
		"Get detailed information about a specific feature flag, including its status across all environments and any configured strategies.",
		GetFeatureStatusSchema.shape,
		async ({ instance, name, project, environment }) => {
			try {
				const unleashInstance = getUnleashInstance(instance);
				const projectName = getProjectName(unleashInstance, project);
				const envName = getEnvironmentName(unleashInstance, environment);
				...
				if (!feature) {
					return createErrorResponse(
						`Feature flag '${name}' not found in ${instance} (${projectName} project)`,
					);
				}
				...
				return createErrorResponse(
					`Failed to get feature status: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		},
	);
	...
	server.tool(
		"toggle_feature",
		"Enable or disable a feature flag in a specific environment (e.g., enable in staging, disable in production). This is how you control where features are active.",
		ToggleFeatureSchema.shape,
		async ({ instance, name, environment, enabled, project }) => {
			try {
				const unleashInstance = getUnleashInstance(instance);
				const projectName = getProjectName(unleashInstance, project);
				...
				const result = await makeUnleashRequest(
					unleashInstance,
					`/projects/${projectName}/features/${name}/environments/${environment}/${action}`,
					{
						method: "POST",
					},
				);
				...
				if (!result) {
					return createErrorResponse(
						`Failed to ${enabled ? "enable" : "disable"} feature flag`,
					);
				}

				const status = enabled ? "✅ ENABLED" : "❌ DISABLED";
				return createSuccessResponse(
					`${status} Feature flag '${name}' ${enabled ? "enabled" : "disabled"} in ${environment} environment (${instance} - ${projectName} project)`,
				);
			} catch (error) {
				console.error("Error toggling feature:", error);
				return createErrorResponse(
					`Failed to toggle feature flag: ${error instanceof Error ? error.message : "Unknown error"}`,
				);
			}
		},
	);
	...
	server.tool(
		"list_environments",
		"Show all available environments (e.g., development, staging, production) for a specific Unleash instance. Use this to see what environments you can deploy features to.",
		{
			instance: InstanceNameSchema,
			project: ProjectNameSchema,
		},
		async ({ instance, project }) => {
			try {
				...
				}>(unleashInstance, `/projects/${projectName}/environments`);
				...
				return createSuccessResponse(
					`Environments in ${instance} (${projectName} project):\n\n${envList}`,
				);
			} catch (error) {
