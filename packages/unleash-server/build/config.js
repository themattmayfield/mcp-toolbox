...
export function loadConfig() {
    ...
    throw new Error("No Unleash configuration found. Please run 'bun run setup' to configure your instances, " +
        "or set environment variables (UNLEASH_URL, UNLEASH_TOKEN) or create a config file.");
}
export function saveConfigToFile(config, configPath) {
...
export function generateEnvVariables(config) {
...
export function generateClaudeDesktopConfig(config) {
    if (config.instances.length === 1) {
        // Simple single instance format
        const instance = config.instances[0];
